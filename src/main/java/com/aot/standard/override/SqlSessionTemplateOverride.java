package com.aot.standard.override;
...
public class SqlSessionTemplateOverride extends SqlSessionTemplate {
    ...
    public int delete(final String statement, final Object parameter) throws CommonException {
        ...
        // if (result == 0) {
        // LOGGER.warn(CommonResponseException.EXCEPTION_ERROR_NO_DATA_SUCCESS.getJsonObject().toString());
        // throw CommonResponseException.EXCEPTION_ERROR_NO_DATA_SUCCESS;
        // }
    ...
    private <T> void writeLog(final T result) {
        try {
            LOGGER.debug(AotMapperUtils.writeObjectAsString(result));
    ...
    private Object convertJsonToMap(final Object parameter) {
        if (parameter != null && parameter instanceof JsonObject) {
            return AotMapperUtils.writeObjectAsHashMap(parameter);
        } else {
            return parameter;
        }
    }
