package com.aot.standard.context.aop.logging.encoder;
...
public class LoggingLayout extends LayoutBase<ILoggingEvent> {
    ...
    public String doLayout(final ILoggingEvent event) {
        ...
        if (StringUtils.endsWith(event.getLoggerName(), "LoggingInAOP") && StringUtils.equals(event.getLevel().levelStr, "INFO")
                || StringUtils.endsWith(event.getLoggerName(), "AitTestUtils") && StringUtils.equals(event.getCallerData()[0].getMethodName(), "logReponse")
    ...
    private void getSql(final ILoggingEvent event, final StringBuffer sbuf) {
        ...
        if (formattedMessage.length() > 20000) {
            sbuf.append("\n" + SQLUtils.formatOracle(formattedMessage) + "\n");
        } else {
            sbuf.append("\n" + StringUtils.abbreviate(formattedMessage, STR_SKIP_TOO_LONG_TEXT, 20000) + "\n");
        }
        sbuf.append(CoreConstants.LINE_SEPARATOR);
    }

    private void getMessageHeader(final ILoggingEvent event, final StringBuffer sbuf) {
        sbuf.append(AotDateUtils.getString(event.getTimeStamp(), "HH:mm:ss.SSS"));
        sbuf.append(" [");
