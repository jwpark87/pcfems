<%----------------------------------------------------------------------------------------
 - 파일이름	: setting/gt/List.jsp
 - 설      명	: Gt 관리 화면
 - 추가내용	:
 - 버전관리	: 1.0
 ----------------------------------------------------------
 -   Date      Version   Auther   Description
 ------------  -----------  ---------  --------------------------
 - 2018.03.02    1.0      KYM      신규작성
------------------------------------------------------------------------------------------%>
<%@page contentType="text/html;charset=utf-8" errorPage="/WEB-INF/views/common/error/catchException.jsp" %>
<%@include file="/WEB-INF/views/common/include.jsp" %>
<aot:html title="${TITLE}" jqGrid="YES">
    <body>
    <c:import url="${CONTEXT_PATH }/top.do"/>
    <c:import url="${CONTEXT_PATH }/left.do"/>
    <form name="myForm" method="POST" action="${CONTEXT_PATH}/setting/gt/gt.do">
        <input type="hidden" name="menuUrl" value="${MENU_URL}"/>
        <input type="hidden" name="srchSidx"/>
        <input type="hidden" name="srchSord"/>
        <input type="hidden" name="gt" id="gt"/>
        <input type="hidden" name="actionID">
        <div id="main" role="main">
            <!-- RIBBON -->
            <div id="ribbon">
                <!-- breadcrumb -->
                <ol class="breadcrumb">
                    <aot:navi menuUrl="${MENU_URL}"></aot:navi>
                </ol>
                <!-- end breadcrumb -->
            </div>
            <div id="content">
                <!-- 상단화면명 START -->
                <div class="row">

                    <!-- 상단화면명 -->
                    <div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
                        <h1 class="page-title txt-color-blueDark">
                            <!-- PAGE HEADER -->
                            <i class="fa-fw fa fa-pencil-square-o"></i>
                                ${curr_menu_name }
                        </h1>
                    </div>

                    <!-- 상단 버튼들 -->
                    <div class="col-lg-4" style="float: right; height: 66px; padding-top: 10px; text-align: right;">

                        <a href="#" data-target="#gtExcelModal" class="btn btn-primary btn-sm">
                            <spring:message code="TEXT.COMM.BTN.EXCEL_UPLOAD"/>
                        </a>
                        <a href="#" data-target="#gtInsertModal" class="btn btn-primary btn-sm">
                            <spring:message code="TEXT.COMM.BTN.REG"/>
                        </a>
                        <a href="#" data-target="#gtModModal" class="btn btn-primary btn-sm">
                            <spring:message code="TEXT.COMM.BTN.UPDATE"/>
                        </a>
                        <button type="button" class="btn btn-primary btn-sm" id="gtDel">
                            <spring:message code="TEXT.COMM.BTN.DELETE"/>
                        </button>
                        <button type="button" class="btn btn-primary btn-sm" id="excel">
                            <spring:message code="TEXT.COMM.BTN.EXCEL"/>
                        </button>
                    </div>

                </div>
                <!-- 상단화면명 END -->

                <!-- 검색박스 START -->
                    <%-- <div class="row">
                    <div class="col-sm-12">
                        <div class="well">
                            <div class="smart-form">

                                    <section class="col col-2">
                                        <label class="input">
                                            <input type="text" name="srch_db_name" id="srch_db_name" placeholder="DB NAME">
                                        </label>
                                    </section>

                                    <section class="col col-2">
                                        <label class="input">
                                            <input type="text" name="srch_gt" id="srch_gt" placeholder="GT">
                                        </label>
                                    </section>

                                    <section class="col col-2">
                                        <label class="input">
                                            <input type="text" name="srch_pc_list_nm" id="srch_pc_list_nm" placeholder="PC List Name">
                                        </label>
                                    </section>

                                    <section class="col col-2">
                                        <label class="input">
                                            <input type="text" name="srch_country" id="srch_country" placeholder="COUNTRY">
                                        </label>
                                    </section>

                                    <section class="col col-2">
                                        <label class="input">
                                            <input type="text" name="srch_carrier_nm" id="srch_carrier_nm" placeholder="사업자명">
                                        </label>
                                    </section>

                                    <button type="button" class="btn btn-primary btn-sm" id="srch">
                                        <spring:message code="TEXT.COMM.BTN.SEARCH"/>
                                    </button>
                            </div>
                        </div>
                    </div>
                </div> --%>
                <!-- 검색박스 END -->

                <section id="widget-grid" class="">
                    <!-- 위젯 아이디 숫자를 맞춰야 순서대로 나옴. -->
                    <div class="row">
                        <article class="col-sm-12">
                            <div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-0" data-widget-custombutton="false" data-widget-colorbutton="false" data-widget-editbutton="false"
                                 data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-togglebutton="false" data-widget-sortable="false">
                                <header></header>
                                <div>
                                    <div class="widget-body no-padding">
                                        <table id="gridList"></table>
                                        <div id="gridToolbar"></div>
                                    </div>
                                </div>
                            </div>
                        </article>
                    </div>
                </section>

            </div>
            <!-- content -->

        </div>
        <!-- main -->
    </form>

    <aot:select name="gd_dsbd_yn" id="gd_dsbd_yn" group="W001" init="YES" initCode="" initName="" style="display:none;width:110px;"></aot:select>

    <c:import url="${CONTEXT_PATH }/footer.do"/>
    <script type="text/javascript">
        $(document).ready(function () {
            bindClickEvent();

            jQuery("#gridList").jqGrid({
                url: "${CONTEXT_PATH}/setting/gt/gt.do",
                postData: {
                    actionID: "ACTION_GT_LIST"
                    /* ,srch_db_name : $("#srch_db_name").val()
                    ,srch_gt: $("#srch_gt").val()
                    ,srch_pc_list_nm: $("#srch_pc_list_nm").val()
                    ,srch_country : $("#srch_country").val()
                    ,srch_carrier_nm: $("#srch_carrier_nm").val() */
                },
                page: "${SEARCH_DATA.jqPage}",
                datatype: "json",
                jsonReader: {
                    root: "root",
                    page: "page",
                    records: "records",
                    repeatitems: false
                },
                mtype: 'POST',
                autoencode: true,
                height: 460,
                colNames: ["<spring:message code="TEXT.SETTING.GT.GRID.DB_NAME"/>",
                    //"GT",
                    "<spring:message code="TEXT.SETTING.GT.GRID.GT"/>",
                    //"PC LIST NM",
                    "<spring:message code="TEXT.SETTING.GT.GRID.PC_LIST_NM"/>", "<spring:message code="TEXT.SETTING.GT.GRID.COUNTRY"/>",
                    //"사업자",
                    "<spring:message code="TEXT.SETTING.GT.GRID.CARRIER_NM"/>", "<spring:message code="TEXT.SETTING.GT.GRID.DSBD_YN"/>"

                    /*   //"작업자",
                      "<spring:message code="TEXT.SETTING.GT.GRID.UPD_ID"/>",
				  //"작업일시"
				  "<spring:message code="TEXT.SETTING.GT.GRID.UPD_DT"/>" */
                ],
                colModel: [{
                    name: "db_name",
                    index: "db_name",
                    align: "left",
                    width: 200
                }, {
                    name: "gt",
                    index: "gt",
                    align: "left",
                    width: 200
                }, {
                    name: "pc_list_nm",
                    index: "pc_list_nm",
                    align: "left",
                    width: 200
                }, {
                    name: "country",
                    index: "country",
                    align: "left",
                    width: 200
                }, {
                    name: "carrier_nm",
                    index: "carrier_nm",
                    align: "left",
                    width: 200
                }, {
                    name: "dsbd_yn",
                    index: "dsbd_yn",
                    align: "left",
                    width: 100,
                    stype: "select",
                    searchoptions: {
                        value: AotJqGrid.getSelectTagData($("#gd_dsbd_yn > option"))
                    },
                    edittype: "select",
                    editoptions: {
                        value: AotJqGrid.getSelectTagData($("#gd_dsbd_yn > option"))
                    },
                    formatter: 'select'
                }
                    /* {name:"upd_id"				,index:"upd_id"				,align:"left"  ,width:150},
                    {name:"upd_dt"				,index:"upd_dt"			,align:"center",width:150} */
                ],
                sortname: "gt",
                rowNum: 100,
                rowList: [100, 300, 500, 1000],
                sortorder: "asc",
                shrinkToFit: true,
                toolbarfilter: true,
                viewrecords: true,
                pager: "#gridToolbar",
                autowidth: true,
                onSelectRow: function (id) {
                    var id = jQuery("#gridList").getGridParam("selrow");
                    if (id) {
                        var ret = jQuery("#gridList").getRowData(id);
                        var gt = ret.gt;
                        deatilInfo(gt);
                    }
                },
                ondblClickRow: function (rowId) {
                    showUpdatePop();
                },
            });
            jQuery("#gridList").jqGrid("navGrid", "#gridToolbar", {
                    edit: false,
                    add: false,
                    del: false,
                    view: false,
                    search: false,
                    refresh: false
                },// edit,add,delete,view,search,options
                {}, {}, {}, {}, {
                    width: 500
                });

            jQuery("#gridList").jqGrid("filterToolbar", {
                stringResult: true,
                searchOnEnter: true,
                defaultSearch: "cn",
                ignoreCase: true
            });
            AotJqGrid.setGridStyle();
        });

        function bindClickEvent() {
            //클릭이벤트
            $("#srch").on('click', function () {
                goGtList();
            });

            /*
            $("#srch_db_name").on('keydown', function (key) {
              if(key.keyCode == 13){
                  goGtList();
              }
            });

            $("#srch_gt").on('keydown', function (key) {
              if(key.keyCode == 13){
                  goGtList();
              }
            });
            $("#srch_pc_list_nm").on('keydown', function (key) {
              if(key.keyCode == 13){
                  goGtList();
              }
            });

            $("#srch_country").on('keydown', function (key) {
              if(key.keyCode == 13){
                  goGtList();
              }
            });

            $("#srch_carrier_nm").on('keydown', function (key) {
              if(key.keyCode == 13){
                  goGtList();
              }
            }); */

            //삭제
            $("#gtDel").on('click', function () {
                if ($("#gt").val() == "") {
                    AotSmartAdmin.smallBoxWarning('<spring:message code="MSG.NONE.SELECT"/>');
                } else {
                    AotSmartAdmin.confirmBox("<spring:message code='MSG.CONFIRM.DELETE'/>", function () {
                        delGt();
                    });
                }
            });

            //등록 모달 호출
            $('a[data-target="#gtInsertModal"]').on('click', function (ev) {
                ev.preventDefault();
                showInsertPop();
            });

            //수정 모달 호출
            $('a[data-target="#gtModModal"]').on('click', function (ev) {
                ev.preventDefault();
                if ($("#gt").val() == "") {
                    AotSmartAdmin.smallBoxWarning('<spring:message code="MSG.NONE.SELECT"/>');
                    $("#gtModModal").modal("hide");
                } else {
                    showUpdatePop();
                }
            });

            $('a[data-target="#gtExcelModal"]').on('click', function (ev) {
                ev.preventDefault();
                showExcelPop();
            });
            //엑셀 다운
            $("#excel").on('click', function () {
                doExcel();
            });
        }

        function showExcelPop() {
            $(this).attr('href', '${CONTEXT_PATH}/setting/gt/gtexcelpop.pop');

            // load the url and show modal on success
            $("#gtExcelModal").load($(this).attr("href"), function () {
                $("#gtExcelModal").modal();
            });
        }

        function showInsertPop() {
            $(this).attr('href', '${CONTEXT_PATH}/setting/gt/gtInsertPopup.do');

            // load the url and show modal on success
            $("#gtInsertModal").load($(this).attr("href"), function () {
                $("#gtInsertModal").modal();
            });
        }

        function showUpdatePop() {
            var param = {
                gt: $("#gt").val()
            };
            $(this).attr('href', '${CONTEXT_PATH}/setting/gt/gtModPopup.do');

            // load the url and show modal on success
            $("#gtModModal").load($(this).attr("href"), param, function () {
                $("#gtModModal").modal();
            });
        }

        //삭제
        function delGt() {
            AotAjax.excute("${CONTEXT_PATH}/setting/gt/gt.do", {
                actionID: "ACTION_GT_DELETE_OK",
                gt: $("#gt").val()
            }, {
                autoResultFunctionTF: false
            }).done(function (response) {
                var str = response.returnMsg.split(":");
                if (str[0] === "success") {
                    AotSmartAdmin.smallBoxsuccess(str[1]);
                    $("#gt").val("");
                    goGtList();
                } else {
                    AotSmartAdmin.smallBoxWarning(str[1]);
                }
            });
        }

        function deatilInfo(gt) {
            $("#gt").val(gt);
        }

        //Realm별 국가 관리정보 취득
        function goGtList() {
            $("#gridList").jqGrid('clearGridData');
            jQuery("#gridList").setGridParam({
                url: "${CONTEXT_PATH}/setting/gt/gt.do?v=" + moment().valueOf(),
                postData: {
                    actionID: "ACTION_GT_LIST"
                    /* ,srch_db_name : $("#srch_db_name").val()
                    ,srch_gt: $("#srch_gt").val()
                    ,srch_pc_list_nm: $("#srch_pc_list_nm").val()
                    ,srch_country : $("#srch_country").val()
                    ,srch_carrier_nm: $("#srch_carrier_nm").val() */
                }
            }).trigger("reloadGrid");
        }

        //엑셀
        function doExcel() {
            if ($("#gridList").getGridParam("reccount") == 0) {
                AotSmartAdmin.smallBoxWarning("<spring:message code='MSG.ALERT.NO_DATA'/>");
                return;
            }
            myForm.srchSidx.value = jQuery("#gridList").getGridParam('sortname');
            myForm.srchSord.value = jQuery("#gridList").getGridParam('sortorder');
            var param = AotCommon.formToJsonParam(myForm);
            param['actionID'] = 'ACTION_GET_EXCEL';
            AotCommon.submitFormPOST('${CONTEXT_PATH}/setting/gt/gt.do', param);
        }
    </script>

    <!-- 모달 창에 아이디 값들은 본화면과 겹지면 안됨 -->
    <!-- content 까지 유지하라는 글을 봤는데 창을 열면 없어지는 현상때문에 팝업 화면에 이동시킴. -->
    <div class="modal fade" id="gtInsertModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>
    <div class="modal fade" id="gtModModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>
    <div class="modal fade" id="gtExcelModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"></div>
    </body>
</aot:html>
