...
/*
 * APP DOM REFERENCES Description: Obj DOM reference, please try to avoid changing these
 */
...
thisDevice = null,
/*
 * DETECT MOBILE DEVICES Description: Detects mobile device - if any of the listed device is detected a class is inserted to $.root_ and the variable thisDevice is decleard. (so far this is covering
 * most hand held devices)
 */
ismobile = (/iphone|ipad|ipod|android|blackberry|mini|windows\sce|palm/i.test(navigator.userAgent.toLowerCase())),
/*
 * JS ARRAY SCRIPT STORAGE Description: used with loadScript to store script path and file name so it will not load twice
 */
jsArray = {},
/*
 * App Initialize Description: Initializes the app with intApp();
 */
initApp = (function(app) {

	/*
	 * ADD DEVICE TYPE Detect if mobile or desktop
	 */
	...
	/*
	 * CHECK FOR MENU POSITION Scans localstroage for menu position (vertical or horizontal)
	 */
		...
		var smartActions = {
			...
			userLogout : function($this) {
				...
				function logout() {
					...
					var param = AotCommon.formToJsonParam($('#topForm'));
					param['actionID'] = 'LOGOUT_OK';
					AotCommon.submitFormPOST(CONTEXT_PATH + '/login.go', param);
	...
	/*
	 * ACTIVATE NAVIGATION Description: Activation will fail if top navigation is on
	 */
	...
	/*
	 * MISCELANEOUS DOM READY FUNCTIONS Description: fire with jQuery(document).ready...
	 */
...
/*
 * RESIZER WITH THROTTLE Source: http://benalman.com/code/projects/jquery-resize/examples/resize/
 */
	...
	function loopy() {
		timeout_id = window[str_setTimeout](function() {
			elems.each(function() {
				...
				var elem = $(this), data = $.data(this, str_data); // width = elem.width(), height = elem.height();
...
/*
 * ADD CLASS WHEN BELOW CERTAIN WIDTH (MOBILE MENU) Description: tracks the page min-width of #CONTENT and NAV when navigation is resized. This is to counter bugs for minimum page width on many
 * desktop and mobile devices. Note: This script utilizes JSthrottle script so don't worry about memory/CPU usage
 */
...
/*
 * DETECT IE VERSION Description: A short snippet for detecting versions of IE in JavaScript without resorting to user-agent sniffing RETURNS: If you're not in IE (or IE version is less than 5) then:
 * //ie === undefined
 * 
 * If you're in IE (>=5) then you can determine which version: // ie === 7; // IE7
 * 
 * Thus, to detect IE: // if (ie) {}
 * 
 * And to detect the version: ie === 6 // IE6 ie > 7 // IE8, IE9 ... ie < 9 // Anything less than IE9
 */
...
/*
 * ELEMENT EXIST OR NOT Description: returns true or false Usage: $('#myDiv').doesExist();
 */
...
/*
 * INITIALIZE FORMS Description: Select2, Masking, Datepicker, Autocomplete
 */
function runAllForms() {

	/*
	 * BOOTSTRAP SLIDER PLUGIN Usage: Dependency: js/plugin/bootstrap-slider
	 */
	if ($.fn.slider) {
		$('.slider').slider();
	}
	...
	/*
	 * SELECT2 PLUGIN Usage: Dependency: js/plugin/select2/
	 */
	if ($.fn.select2) {
	...
	/*
	 * MASKING Dependency: js/plugin/masked-input/
	 */
	if ($.fn.mask) {
		$('[data-mask]').each(function() {

			var $this = $(this), mask = $this.attr('data-mask') || 'error...', mask_placeholder = $this.attr('data-mask-placeholder') || 'X';
	...
	/*
	 * AUTOCOMPLETE Dependency: js/jqui
	 */
	if ($.fn.autocomplete) {
		$('[data-autocomplete]').each(function() {

			var $this = $(this), availableTags = $this.data('autocomplete') || [ "The", "Quick", "Brown", "Fox", "Jumps", "Over", "Three", "Lazy", "Dogs" ];
	...
	/*
	 * JQUERY UI DATE Dependency: js/libs/jquery-ui-1.10.3.min.js Usage: <input class="datepicker" />
	 */
	if ($.fn.datepicker) {
		$('.datepicker').each(function() {

			var $this = $(this), dataDateFormat = $this.attr('data-dateformat') || 'dd.mm.yy';
	...
	/*
	 * AJAX BUTTON LOADING TEXT Usage: <button type="button" data-loading-text="Loading..." class="btn btn-xs btn-default ajax-refresh"> .. </button>
	 */
...
function runAllCharts() {
	/*
	 * SPARKLINES DEPENDENCY: js/plugins/sparkline/jquery.sparkline.min.js See usage example below...
	 */

	/*
	 * Usage: <div class="sparkline-line txt-color-blue" data-fill-color="transparent" data-sparkline-height="26px"> 5,6,7,9,9,5,9,6,5,6,6,7,7,6,7,8,9,7 </div>
	 */
	...
	/*
	 * EASY PIE CHARTS DEPENDENCY: js/plugins/easy-pie-chart/jquery.easy-pie-chart.min.js Usage: <div class="easy-pie-chart txt-color-orangeDark" data-pie-percent="33" data-pie-size="72"
	 * data-size="72"> <span class="percent percent-sign">35</span> </div>
	 */
	...
	if ($.fn.easyPieChart) {
		...
		$('.easy-pie-chart').each(
				function() {
					var $this = $(this), barColor = $this.css('color') || $this.data('pie-color'), trackColor = $this.data('pie-track-color') || 'rgba(0,0,0,0.04)', size = parseInt($this
							.data('pie-size')) || 25;
...
/*
 * GOOGLE MAPS description: Append google maps to head dynamically (only execute for ajax version) Loads at the begining for ajax pages
 */
...
/*
 * LOAD SCRIPTS Usage: Define function = myPrettyCode ()... loadScript("js/my_lovely_script.js", myPrettyCode);
 */
...
/*
 * APP AJAX REQUEST SETUP Description: Executes and fetches all ajax requests also updates naivgation elements to active
 */
if ($.navAsAjax) {
	...
	$.ajax({
		...
		beforeSend : function() {
			...
			if ($.navAsAjax && $(".google_maps")[0] && (container[0] == $("#content")[0])) {
				...
				var collection = $(".google_maps"), i = 0;
				// run for each map
			...
			if ($.navAsAjax && (container[0] == $("#content")[0])) {
				 ...
				 * 
				 * colorList icon picker inline And unbind events from elements:
				 * 
				 * icon picker inline especially $(document).on('mousedown') It will be much easier to add namespace to plugin events and then unbind using selected namespace.
				 * 
				 * See also:
				 * 
				 * http://f6design.com/journal/2012/05/06/a-jquery-plugin-boilerplate/ http://keith-wood.name/pluginFramework.html
				 */
		...
		error : function(xhr, status, thrownError, error) {
			container.html('<h4 class="ajax-loading-error"><i class="fa fa-warning txt-color-orangeDark"></i> Error requesting <span class="txt-color-red">' + url + '</span>: ' + xhr.status
					+ ' <span style="text-transform: capitalize;">' + thrownError + '</span></h4>');
		},
		async : true
	});
...
/*
 * PAGE SETUP Description: fire certain scripts that run through the page to check for form elements, tooltip activation, popovers, etc...
 */
function pageSetUp() {
...
/*
 * ONE POP OVER THEORY Keep only 1 active popover per trigger - also check and hide active popover if user clicks on document
 */
...
/*
 * DELETE MODEL DATA ON HIDDEN Clears the model data once it is hidden, this way you do not create duplicated data on multiple modals
 */
...
/*
 * HELPFUL FUNCTIONS We have included some functions below that can be resued on various occasions
 * 
 * Get param value example: alert( getParam( 'param' ) );
 */
