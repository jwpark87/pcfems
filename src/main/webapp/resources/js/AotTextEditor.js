...
AotTextEditor = {
    ...
    init: function (targetObj) {
        ...
        $(targetObj).find('[data-texteditor="on"]').each(function () {
            var options = {};
            if (AotCommon.isNotEmpty($(this).attr('data-texteditor-height'))) {
                options['height'] = $(this).attr('data-texteditor-height') + 'px';
            }
            if ($(this).attr('data-texteditor-airmode') === 'true') {
                options['airMode'] = true;
            }
            AotTextEditor.instance(this, options);
        });
    },
    instance: function (targetObj, paramOptions) {
        // 서버 이미지 업로드 지원
        // if (!paramOptions.useBase64DataUrlImage && !AotCommon.isLocalhost()) {
        // paramOptions['callbacks'] = {
        // onImageUpload : function(files) {
        // var $summernote = $(this);
        // var formData = new FormData();
        // formData.append("file", files[0]);
        // AotAjax.excuteWithFile(CONTEXT_PATH + '/common/textEditor/imageUpload.proc', formData).done(function(response) {
        ...
        // }
        // }
        $(targetObj).summernote(AotCommon.getOptions(AotTextEditor.defaultOptions, paramOptions));
    },
    ...
    getLength: function (targetObj) {
        return AotTextEditor.getCode(targetObj).length;
    },
